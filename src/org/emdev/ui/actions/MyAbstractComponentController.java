 ...
 */
public abstract class MyAbstractComponentController<ManagedComponent> implements MyIActionController<ManagedComponent> {

    protected static final LogContext LCTX = LogManager.root().lctx("Actions");

    protected final Map<Integer, MyActionEx> m_actions = new LinkedHashMap<Integer, MyActionEx>();

    protected final ReentrantReadWriteLock m_actionsLock = new ReentrantReadWriteLock();

    protected final MyIActionController<?> m_parent;
     ...
     *            managed component
     */
    protected MyAbstractComponentController(final ManagedComponent managedComponent) {
     ...
     *            managed component
     */
    protected MyAbstractComponentController(final MyIActionController<?> parent, final ManagedComponent managedComponent) {
        m_parent = parent;
    ...
    public void setManagedComponent(final ManagedComponent component) {
        ...
        try {
            ...
            for (final MyActionEx action : m_actions.values()) {
     ...
     */
    @Override
    public MyActionEx getAction(final int id) {
        m_actionsLock.readLock().lock();
        try {
            MyActionEx actionEx = m_actions.get(id);
     ...
     */
    @Override
    public MyActionEx getOrCreateAction(final int id) {
        MyActionEx result = null;
        m_actionsLock.writeLock().lock();
        try {
            result = getAction(id);
            if (result == null) {
     ...
     */
    @Override
    public MyActionEx createAction(final int id, final IActionParameter... parameters) {
        final MyActionEx result = new MyActionEx(this, id);
    ...
    public final MyActionEx setActionForView(final View view) {
        final MyActionEx action = getOrCreateAction(view.getId());
