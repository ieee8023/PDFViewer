...
public final class MyGLView extends GLRootView implements MyIView, SurfaceHolder.Callback {
    ...
    protected MyScrollEventThread scrollThread;
    ...
    public MyGLView(final Activity baseActivity) {
        super(((MYIActivityController)baseActivity).getContext());
        ...
        fullScreenCallback = FullScreenCallback.get(baseActivity, this);

        drawQueue = new DrawQueue();
        scrollThread = new MyScrollEventThread((MYIActivityController) base, this);
        scrollThread.start();
    }
    ...
    protected void draw(GLCanvas canvas) {
        ...
        if (viewState == null) {
            viewState = ViewState.get(((MYIActivityController) base).getDocumentController());
     ...
     */
    @Override
    public final MYIActivityController getBase() {
        return (MYIActivityController) base;
    }
        ...
        try {
            ...
            if(ev.getAction() == MotionEvent.ACTION_DOWN) {
                ...
                if (Math.abs(deltaX) < MIN_DISTANCE) {
                	if(searchOpen) {
                		searchOpen = false;
                		base.getActionBar().hide();;
                	} else {
                		searchOpen = true;
                		base.getActionBar().show();;
                	}
                }
            } 
            
            if (((MYIActivityController) base).getDocumentController().onTouchEvent(ev)) {
                return true;
            }
    ...
    protected final void onLayout(final boolean layoutChanged, final int left, final int top, final int right,
        ...
        ((MYIActivityController) base).getDocumentController().onLayoutChanged(layoutChanged, layoutLocked, oldLayout, layout.get());
    ...
    public float getScrollScaleRatio() {
        final Page page = ((MYIActivityController) base).getDocumentModel().getCurrentPageObject();
        if (page == null) {
            return 0;
        }

        final float zoom = ((MYIActivityController) base).getZoomModel().getZoom();
    ...
    public final void redrawView() {
        redrawView(ViewState.get(((MYIActivityController) base).getDocumentController()));
    }
    ...
    public final void redrawView(final ViewState viewState) {
        if (viewState != null) {
            ...
            final DecodeService ds = ((MYIActivityController) base).getDecodeService();
            if (ds != null) {
